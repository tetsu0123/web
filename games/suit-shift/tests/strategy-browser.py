...
report={'version':'2.1.0','checks':[],'viewports':[],'consoleErrors':[],'limitations':['Chromium uses set_content and an in-memory storage test double. Physical iOS/Android devices, real persisted browser storage and HTTP offline lifecycle are not covered.']}
with sync_playwright() as p:
 ...
 print('checkpoint',len(report['checks'])+1,flush=True);report['checks'].append('Real pointer gesture, shared-suit selection, two-pair preview and clear')
 ...
 print('checkpoint',len(report['checks'])+1,flush=True);report['checks'].append('Exact budget exhaustion fails; extra actions blocked; a measured greedy route cannot clear')
 ...
 print('checkpoint',len(report['checks'])+1,flush=True);report['checks'].append('Failure and spent undo survive reload; undo limited to one and refunds only that move')
 ...
 assert page.evaluate('SuitShift.getState().hintsUsed')==1
 print('checkpoint',len(report['checks'])+1,flush=True);report['checks'].append('Single successful hint consumes its allowance and undo does not restore it')
 ...
 print('checkpoint',len(report['checks'])+1,flush=True);report['checks'].append('Assisted clear recorded without unassisted shortest medal')
 ...
 for i in range(120):
  if i%20==0:print('replay',i,flush=True)
 ...
 print('checkpoint',len(report['checks'])+1,flush=True);report['checks'].append('All 120 shortest routes clear in app; winning on the final permitted move takes precedence over failure')
 ...
 print('checkpoint',len(report['checks'])+1,flush=True);report['checks'].append('Unlimited practice allows longer valid route, with records separate from challenge')
 ...
 assert page.evaluate('SuitShift.getState().moves')==1
 print('checkpoint',len(report['checks'])+1,flush=True);report['checks'].append('Mode confirmation, direction buttons and casino color variants work')
 ...
 for w,h in [(320,568),(360,640),(390,844),(430,932),(768,1024),(1440,1000)]:
  print('viewport',w,h,flush=True);page.set_viewport_size({'width':w,'height':h});page.wait_for_timeout(80)
 ...
 print('checkpoint',len(report['checks'])+1,flush=True);report['checks'].append('Malformed assistance state rejected; old puzzle records retained separately without false v2 awards')
